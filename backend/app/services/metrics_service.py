        ...
        try:
            ...
            for t in fact_tables:
                ...
                if amount_col:
                    try:
                        ...
                        safe_col = db_connector.validate_identifier(amount_col)
                        safe_table = db_connector.validate_identifier(t_name)
                        sql = f"SELECT AVG({safe_col}) as avg_val FROM {safe_table}"
            ...
            for t in fact_tables:
                ...
                if ts_col:
                    try:
                        # parameterized below
                        span_sql = f"SELECT EXTRACT(EPOCH FROM (MAX({db_connector.validate_identifier(ts_col)}) - MIN({db_connector.validate_identifier(ts_col)}))) as span_seconds FROM {t_name}"
