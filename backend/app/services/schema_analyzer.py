...
Introspects connected database schemas to extract tables, columns, row counts, and foreign key relationships.
"""
import asyncio
import logging
import time
        ...
        for t_row in tables_data:
            ...
            if r_count is None or r_count == 0:
                try:
                    ...
                    safe_t = db_connector.validate_identifier(t_name)
                    count_q = f"SELECT COUNT(*) as c FROM {safe_t}"
