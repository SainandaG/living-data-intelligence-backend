...
class DatabaseSeeder:
    ...
    async def _seed_users(self, connection_id: str, count: int, is_mysql: bool = False):
        ...
        for i in range(count):
            ...
            if not is_mysql:
                sql = f'INSERT INTO {prefix}users (username, email, created_at) VALUES (%s, %s, %s) ON CONFLICT DO NOTHING'
            else:
                sql = f'INSERT IGNORE INTO {prefix}users (username, email, created_at) VALUES (%s, %s, %s)'
    ...
    async def _seed_products(self, connection_id: str, count: int, is_mysql: bool = False):
        ...
        for i in range(count):
            ...
            sql = f'INSERT INTO {prefix}products (name, price, created_at) VALUES (%s, %s, %s)'
    ...
    async def _seed_orders(self, connection_id: str, count: int, is_mysql: bool = False):
        prefix = "" if is_mysql else f"{self.schema}."
        user_ids_resp = await db_connector.query(connection_id, f"SELECT id FROM {prefix}users")
        ...
        for i in range(count):
            ...
            sql = f'INSERT INTO {prefix}orders (user_id, total_amount, created_at) VALUES (%s, %s, %s)'
    ...
    async def _seed_transactions(self, connection_id: str, count: int, is_mysql: bool = False):
        prefix = "" if is_mysql else f"{self.schema}."
        order_ids_resp = await db_connector.query(connection_id, f"SELECT id, total_amount FROM {prefix}orders")
        ...
        for i in range(count):
            ...
            sql = f'INSERT INTO {prefix}transactions (order_id, amount, recorded_at) VALUES (%s, %s, %s)'
    ...
    async def _seed_wezu_assets(self, connection_id: str, is_mysql: bool = False):
        ...
        for i in range(15):
            ...
            if not is_mysql:
                sql = f'INSERT INTO {prefix}batteries (serial_number, soh_percentage, voltage, temperature, lifetime_revenue, swap_variance) VALUES (%s, %s, %s, %s, %s, %s) ON CONFLICT DO NOTHING'
            else:
                sql = f'INSERT IGNORE INTO {prefix}batteries (serial_number, soh_percentage, voltage, temperature, lifetime_revenue, swap_variance) VALUES (%s, %s, %s, %s, %s, %s)'
        ...
        for i in range(10):
            if not is_mysql:
                sql = f'INSERT INTO {prefix}iot_devices (device_id, firmware_version, signal_strength) VALUES (%s, %s, %s) ON CONFLICT DO NOTHING'
            else:
                sql = f'INSERT IGNORE INTO {prefix}iot_devices (device_id, firmware_version, signal_strength) VALUES (%s, %s, %s)'
    ...
    async def _seed_wezu_telemetry(self, connection_id: str, is_mysql: bool = False):
        ...
        prefix = "" if is_mysql else f"{self.schema}."
        battery_ids_resp = await db_connector.query(connection_id, f"SELECT id FROM {prefix}batteries")
        ...
        for b_id in batt_ids:
            ...
            for week in range(4):
                ...
                sql = f'INSERT INTO {prefix}battery_health_log (battery_id, soh_percentage, recorded_at) VALUES (%s, %s, %s)'
            ...
            for day in range(30):
                ...
                sql = f'INSERT INTO {prefix}telematics_data (battery_id, voltage, current, temperature, recorded_at) VALUES (%s, %s, %s, %s, %s)'
            ...
            sql = f'INSERT INTO {prefix}gps_tracking_log (battery_id, lat, lng) VALUES (%s, %s, %s)'
            await db_connector.query(connection_id, sql, (b_id, 12.9 + random.random(), 77.5 + random.random()))

        # 4. Swap Transactions (Random history)
        station_ids_resp = await db_connector.query(connection_id, f"SELECT id FROM {prefix}stations")
        station_ids = [r['id'] for r in station_ids_resp]
        if station_ids:
            for _ in range(50):
                sql = f'INSERT INTO {prefix}swap_transactions (battery_id, station_id, recorded_at) VALUES (%s, %s, %s)'
