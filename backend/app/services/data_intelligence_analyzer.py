...
class DataIntelligenceAnalyzer:
    ...
    async def analyze_table_data(self, db_connector, connection_id: str, table_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            sample_data = await self._get_sample_data(db_connector, connection_id, table_name, limit=500)
        ...
        try:
            ...
            if is_mysql:
                ...
                query = """
                    ...
                    ORDER BY ordinal_position
                """
            return await db_connector.query(connection_id, query, (table_name,))
        except Exception as e:
            logger.debug(f"Column info fetch failed for {table_name}: {e}")
            return []
    ...
    async def _get_sample_data(self, db_connector, connection_id: str, table_name: str, limit: int = 10000) -> List[Dict]:
        ...
        try:
            ...
            logger.debug(f"Sample data fetch failed for {table_name}: {e}")
            return []
            ...
            for i, col1 in enumerate(numeric_cols):
                for col2 in numeric_cols[i+1:]:
                    try:
                        ...
                        logger.debug(f"Correlation calc failed for {col1}/{col2}: {e}")
