...
const AuthPage = ({ onLoginSuccess }) => {
    ...
    const handleGoogleSignIn = async () => {
        ...
        try {
            if (!firebaseAuth) {
                throw new Error('Google Sign-In is not configured. Please define the VITE_FIREBASE_* environment variables in your .env file.');
            }
                ...
                {!mfaRequired ? (
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <button
                            ...
                            disabled={loading}
                            className="w-full py-3.5 rounded-xl bg-slate-950/50 border border-slate-800 text-white font-bold text-[10px] tracking-[0.2em] uppercase hover:border-slate-600 hover:bg-slate-900/50 transition-all active:scale-[0.98] disabled:opacity-30 flex items-center justify-center gap-3"
                        >
