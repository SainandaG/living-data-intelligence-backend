...
import { GENERATION_LOG_HIDE_DELAY } from '../../config/timing';
import apiClient from '../../utils/apiClient';
import { logger } from '../../utils/logger';
import { useConnectionStore } from '../../stores/connectionStore';

export const GenerationLogPanel = () => {
    const { connectionId: ctxConnectionId, isGenerationLogVisible, setIsGenerationLogVisible } = useWindowManager();
    const { connectionId: storeConnectionId } = useConnectionStore();
    // Context connectionId is only set by manual modal. Fall back to the Zustand store
    // (which is updated by auto-connect on startup) so the button always works.
    const connectionId = ctxConnectionId || storeConnectionId;
    ...
    useEffect(() => {
        ...
        const fetchHistory = async () => {
            try {
                const resp = await apiClient.get(`/graph/generation-logs/${connectionId}`);
                if (resp.logs) {
                    setLogs(resp.logs);
                    if (resp.logs.length > 0) {
                        const lastLog = resp.logs[resp.logs.length - 1];
                        setProgress(lastLog.progress || 0);
                    }
                }
            } catch (err) {
                logger.error("Failed to fetch log history", err);
            }
        };
        fetchHistory();
    }, [connectionId]);

    // 2. Subscribe to generation logs
    ...
    useEffect(() => {
        if (lastMessage && lastMessage.type === 'generation_log') {
            ...
            // Keep status updated, but don't force show the panel
            setMinimized(false);
    ...
    if (!connectionId || !isGenerationLogVisible) return null;
        ...
        <div className={cn("generation-log-panel glass-card", minimized && 'minimized')}>
            <div className="panel-header" onClick={() => setMinimized(!minimized)}>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button className="text-white/50 hover:text-white" onClick={(e) => { e.stopPropagation(); setIsGenerationLogVisible(false); }}>×</button>
                </div>
            </div>
