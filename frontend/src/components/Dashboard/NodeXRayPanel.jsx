                ...
                <div style={styles.scrollArea}>
                    {loading ? (
                        ...
                        <>
                            ...
                            <Section title="DATA QUALITY" icon={<Shield size={14} />} badge="graph-derived">
                            ...
                            {(timeline.history?.length > 0 || timeline.forecast?.length > 0) && (
                                <Section title="TRANSACTION TIMELINE" badge="forecast" icon={<Clock size={14} />}
                                    ...
                                    <div style={styles.weeklyBanner}>
                                        ...
                                        <div style={styles.weekStat}>
                                            ...
                                            <span className="text-slate-300/60 font-bold text-sm font-mono">
...
function ComparisonView({ node, data, selectedColumns }) {
    ...
    if (compareItems.length === 0) {
        return (
            <div style={styles.errorWrap}>
                <Layers size={24} className="text-indigo-400/40" />
                <p className="text-slate-300/50 text-[11px] mt-2">
...
function ColumnComparisonCard({ name, nodeName, stats }) {
    return (
        <div style={{
            ...
            <div className="mb-2">
                <div className="text-xs font-extrabold text-white font-mono">{name}</div>
                <div className="text-[8px] text-slate-300/40 uppercase">{nodeName}</div>
            </div>
            <div className="flex flex-col gap-1">
                <div className="flex justify-between text-[9px]">
                    <span className="text-slate-300/40">QUALITY</span>
...
function TimelineAreaChart({ history, forecast }) {
    ...
    if (allPoints.length === 0) return <div className="text-slate-300/30 text-[10px] text-center p-5">No timeline data available</div>;
    ...
    return (
        <svg width={W} height={H} className="block w-full h-auto">
            <defs>
...
function DailyBarChart({ history }) {
    ...
    if (!history.length) return <div className="text-slate-300/30 text-[10px] text-center p-5">No daily data</div>;
...
function ForecastLineChart({ history, forecast }) {
    ...
    if (allPoints.length === 0) return <div className="text-slate-300/30 text-[10px] text-center p-5">No data</div>;
...
function Section({ title, icon, subtitle, children, badge }) {
    return (
        <div style={styles.section}>
            <div style={styles.sectionHead}>
                <div className="flex items-center gap-1.5">
                    ...
                    {badge === 'graph-derived' && (
                        <span
                            title="Score derived from graph topology (vitality + degree centrality). Not computed from direct null/duplicate row scans."
                            style={{ fontSize: '8px', color: '#f59e0b', border: '1px solid rgba(245,158,11,0.3)',
                    ...
                    {badge === 'forecast' && (
                        <span
                            title="Forecast uses a heuristic estimate (2%/month baseline) when insufficient historical transaction data is available."
                            style={{ fontSize: '8px', color: '#818cf8', border: '1px solid rgba(129,140,248,0.3)',
                                     borderRadius: '3px', padding: '1px 5px', letterSpacing: '0.05em',
                                     cursor: 'help', fontFamily: 'monospace' }}>
                            est.
                        </span>
    ...
    return (
        <div style={styles.colCard}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4px', fontSize: '9px' }}>
                ...
                {stats.min !== undefined && (
                    <>
                        <div><span style={styles.statLabel}>MIN</span><span className="text-slate-200">{Number(stats.min).toLocaleString(undefined, { maximumFractionDigits: 1 })}</span></div>
                        <div><span style={styles.statLabel}>MAX</span><span className="text-slate-200">{Number(stats.max).toLocaleString(undefined, { maximumFractionDigits: 1 })}</span></div>
    ...
    return (
        <div style={{ marginBottom: '10px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '9px', marginBottom: '4px' }}>
                ...
                <span className="text-slate-300/40">{Number(stats.min).toFixed(1)} — {Number(stats.max).toFixed(1)}</span>
            </div>
