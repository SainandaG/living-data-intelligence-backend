...
/* ── Toggle Switch ───────────────────────────────────────────── */
const ToggleSwitch = React.memo(({ checked, onChange, activeColor }) => (
    <button
        onClick={() => onChange?.(!checked)}
        className={cn(
            'w-7 h-4 rounded-full relative transition-all duration-300 border shrink-0',
            checked
                ? 'border-current/40'
                : 'bg-gray-800 border-white/10',
        )}
        style={checked ? { backgroundColor: `${activeColor}20`, borderColor: `${activeColor}40` } : {}}
    >
        <div
            className={cn(
                'w-2.5 h-2.5 rounded-full absolute top-[1px] transition-all duration-300',
                checked ? 'left-[13px]' : 'left-0.5 bg-gray-500',
            )}
            style={checked ? {
...
export const Legend = React.memo(({ layoutMode, showPKs, showFKs, setShowPKs, setShowFKs, activeLens = 'ops', onToggleLayoutMode, singleNodeViewEnabled, setSingleNodeViewEnabled }) => {
    return (
        <div className="absolute left-4 bottom-[70px] z-40 transition-all duration-500 flex flex-col gap-2">

            {/* ── Visual Settings ───────────────────────────────── */}
            <div className="glass-card p-2 px-3 flex flex-col gap-2 min-w-[148px] border border-white/[0.07] shadow-2xl backdrop-blur-xl">
                <div className="text-[8px] font-bold text-gray-500 uppercase tracking-[0.2em] border-b border-white/5 pb-1 mb-0.5">
                    Key Visibility
                </div>
                ...
                <div className="flex flex-col gap-1.5">
                    <div className="flex items-center justify-between gap-3">
                        <div className="flex items-center gap-1.5">
                            <div className={cn(
                                ...
                                showPKs ? 'bg-[#00ff88]' : 'bg-gray-600',
                            )} style={showPKs ? { boxShadow: '0 0 6px #00ff88' } : {}} />
                            <span className={cn('text-[9px] font-bold tracking-tight', showPKs ? 'text-white' : 'text-gray-500')}>
                                Primary Keys
                            </span>
                        </div>
                        <ToggleSwitch checked={showPKs} onChange={setShowPKs} activeColor="#00ff88" />
                    </div>
                    ...
                    <div className="flex items-center justify-between gap-3">
                        <div className="flex items-center gap-1.5">
                            <div className={cn(
                                'w-1.5 h-1.5 rounded-full shrink-0',
                                showFKs ? 'bg-[#fbbf24]' : 'bg-gray-600',
                            )} style={showFKs ? { boxShadow: '0 0 6px #fbbf24' } : {}} />
                            <span className={cn('text-[9px] font-bold tracking-tight', showFKs ? 'text-white' : 'text-gray-500')}>
                                Foreign Keys
                            </span>
                        </div>
                        <ToggleSwitch checked={showFKs} onChange={setShowFKs} activeColor="#fbbf24" />
                    </div>
                </div>
                ...
                <div className="mt-1 flex flex-col gap-1.5">
                    <button
                        onClick={() => onToggleLayoutMode && onToggleLayoutMode(layoutMode === 'spin_expand' ? 'galaxy' : 'spin_expand')}
                        className={cn(
                            "w-full flex items-center justify-center gap-2 py-2 rounded-lg text-[9px] font-bold tracking-[0.1em] transition-all duration-300",
                            layoutMode === 'spin_expand'
                                ? "bg-amber-500 text-black shadow-[0_0_15px_rgba(245,158,11,0.4)]"
                                : "bg-amber-500/10 text-amber-500 border border-amber-500/20 hover:bg-amber-500/20"
                        )}
                    >
                        <Layers size={11} strokeWidth={2.5} />
                        {layoutMode === 'spin_expand' ? 'DEACTIVATE SPIN' : 'SPIN & EXPAND'}
                    </button>

                    {/* ── Single Node View (only in spin_expand mode) ── */}
                    {layoutMode === 'spin_expand' && (
                        <button
                            onClick={() => setSingleNodeViewEnabled && setSingleNodeViewEnabled(!singleNodeViewEnabled)}
                            className={cn(
                                "w-full flex items-center justify-center gap-2 py-2 rounded-lg text-[9px] font-bold tracking-[0.1em] transition-all duration-300",
                                singleNodeViewEnabled
                                    ? "bg-purple-500 text-white shadow-[0_0_15px_rgba(168,85,247,0.5)]"
                                    : "bg-purple-500/10 text-purple-400 border border-purple-500/20 hover:bg-purple-500/20"
                            )}
                        >
                            <ScanEye size={11} strokeWidth={2.5} />
                            {singleNodeViewEnabled ? 'NODE VIEW ON' : 'SINGLE NODE VIEW'}
                        </button>
                    )}
                </div>
            </div>
