...
export class GraphSoundEngine {
    private legacySystem: LegacySoundSystem;
    ...
    constructor() {
        ...
        if (FEATURE_FLAGS.ENABLE_ADVANCED_AUDIO) {
            try {
                ...
                logger.warn('Advanced audio failed, using legacy system');
                this.audioContext = null;
            }
        }
    }

    generateSoundFromPath(path: string[]) {
        ...
        if (FEATURE_FLAGS.ENABLE_ADVANCED_AUDIO && this.audioContext) {
            try {
                ...
                logger.warn('Advanced sound failed, using legacy');
            }
        }
    ...
    private generateAdvancedSound(path: string[]) {
