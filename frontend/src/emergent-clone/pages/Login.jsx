...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      else setError('Invalid server response');
    } catch (err) {
    ...
    <div className="bg-[var(--bg-dark)] text-[var(--text-main)] min-h-screen flex items-center justify-center px-4 pt-16">
      ...
      <motion.div initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: 0.1 } } }} className="w-full max-w-sm relative z-10">
        <motion.div variants={fadeUp} className="text-center mb-8">
          ...
          <h1 className="text-2xl font-display font-bold">System Login</h1>
          <p className="text-[var(--text-muted)] text-[10px] font-mono tracking-widest uppercase mt-1">ACCESS_LEVEL_AUTH_v3.2</p>
        </motion.div>
        ...
        <motion.form variants={fadeUp} onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label htmlFor="login-pass" className="block text-sm text-[var(--text-muted)] mb-1">Password</label>
        ...
        <motion.p variants={fadeUp} className="text-center text-sm text-[var(--text-muted)] mt-6">
          ...
          <Link to="/site/signup" className="text-[var(--primary)] hover:text-[var(--primary)]/80 cursor-pointer">Create one</Link>
        </motion.p>
